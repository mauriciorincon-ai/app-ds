...
| 2026-07-11 | Hallazgos e2e: (1) axe `scrollable-region-focusable` en la preview del CSV puntuado ⇒ contenedor con `role="region"` + `tabIndex=0` (ojo: ConfigScreen tiene el mismo patrón sin foco y nunca pasó por axe — anotado para no repetirlo); (2) el route announcer de Next expone `role="alert"` ⇒ los selectores de alertas en e2e filtran por texto; (3) lección D1 otra vez: la clase positiva de marketing-campania es «0» (85 vs 115), no «1» — verificado contra el CSV real. |
| 2026-07-11 | **Gate visual (usuario, sobre la preview):** probó el flujo completo con Titanic — todo funciona; feedback: esperaba "Exportar modelo" también en la pantalla de scoring. Ajuste aplicado pre-merge: botón export en la barra inferior de ScoreScreen SOLO para modelos entrenados en la sesión (un importado ya es el archivo); mismas claves i18n de results.export. Unit 182/182, e2e de la pantalla re-verificados.                                                          |
